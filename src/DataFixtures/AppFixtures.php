...
class AppFixtures extends Fixture
{
    ...
    public function load(ObjectManager $manager): void
    {
        ...
        $users = [
            ['Koffi', 'Ama', 'admin@gmail.com', 'ROLE_ADMIN'],
        ...
        foreach ($users as [$nom, $prenom, $email, $role]) {
            ...
            // On utilise 'password' comme mot de passe pour tout le monde
            $hashedPassword = $this->hasher->hashPassword($u, 'password');
            $u->setPassword($hashedPassword);

            $manager->persist($u);
        }

        $manager->flush(); // N'oublie pas le flush à la fin de la méthode !
    }
}